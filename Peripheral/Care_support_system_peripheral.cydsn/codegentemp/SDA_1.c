...
#include "SDA_1.h"
    ...
    #define SDA_1_SetP4PinDriveMode(shift, mode)  \
    do { \
        SDA_1_PC =   (SDA_1_PC & \
                                (uint32)(~(uint32)(SDA_1_DRIVE_MODE_IND_MASK << \
                                (SDA_1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (SDA_1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define SDA_1_SetP4PinDriveMode(shift, mode)  \
        do { \
            SDA_1_USBIO_CTRL_REG = (SDA_1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(SDA_1_DRIVE_MODE_IND_MASK << \
                                    (SDA_1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (SDA_1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		SDA_1_SetP4PinDriveMode(SDA_1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(SDA_1_DR & (uint8)(~SDA_1_MASK));
    drVal = (drVal | ((uint8)(value << SDA_1_SHIFT) & SDA_1_MASK));
    SDA_1_DR = (uint32)drVal;
}
...
{
    return (uint8)((SDA_1_PS & SDA_1_MASK) >> SDA_1_SHIFT);
}
...
{
    return (uint8)((SDA_1_DR & SDA_1_MASK) >> SDA_1_SHIFT);
}
...
void SDA_1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  SDA_1_INTCFG & (uint32)(~(uint32)position);
    SDA_1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(SDA_1_INTSTAT & SDA_1_MASK);
	SDA_1_INTSTAT = maskedStatus;
    return maskedStatus >> SDA_1_SHIFT;
}
