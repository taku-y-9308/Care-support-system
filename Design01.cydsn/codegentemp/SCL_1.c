...
#include "SCL_1.h"
    ...
    #define SCL_1_SetP4PinDriveMode(shift, mode)  \
    do { \
        SCL_1_PC =   (SCL_1_PC & \
                                (uint32)(~(uint32)(SCL_1_DRIVE_MODE_IND_MASK << \
                                (SCL_1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (SCL_1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define SCL_1_SetP4PinDriveMode(shift, mode)  \
        do { \
            SCL_1_USBIO_CTRL_REG = (SCL_1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(SCL_1_DRIVE_MODE_IND_MASK << \
                                    (SCL_1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (SCL_1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		SCL_1_SetP4PinDriveMode(SCL_1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(SCL_1_DR & (uint8)(~SCL_1_MASK));
    drVal = (drVal | ((uint8)(value << SCL_1_SHIFT) & SCL_1_MASK));
    SCL_1_DR = (uint32)drVal;
}
...
{
    return (uint8)((SCL_1_PS & SCL_1_MASK) >> SCL_1_SHIFT);
}
...
{
    return (uint8)((SCL_1_DR & SCL_1_MASK) >> SCL_1_SHIFT);
}
...
void SCL_1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  SCL_1_INTCFG & (uint32)(~(uint32)position);
    SCL_1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(SCL_1_INTSTAT & SCL_1_MASK);
	SCL_1_INTSTAT = maskedStatus;
    return maskedStatus >> SCL_1_SHIFT;
}
