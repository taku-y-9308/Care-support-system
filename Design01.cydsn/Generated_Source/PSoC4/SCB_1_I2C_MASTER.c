...
#else
    #define SCB_1_I2CMasterDisableEcAm()     do{ /* Empty */ }while(0)
#endif /* (!SCB_1_CY_SCBIP_V0) */

static uint32 SCB_1_I2CMasterHandleStatus(uint32 status);
static uint32 SCB_1_I2CMasterWaitOneUnit(uint32 *timeout);
...
uint32 SCB_1_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    if(SCB_1_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        SCB_1_SetMasterInterruptMode    (SCB_1_I2C_INTR_MASTER_MASK);
...
{
    uint32 errStatus = SCB_1_I2C_MSTR_NOT_READY;
    ...
    if(SCB_1_CHECK_I2C_FSM_IDLE)
    {
        ...
        {
            uint32 locStatus;
            uint32 timeout = SCB_1_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);

            /* Non-empty for master-slave mode when wakeup enabled */
            SCB_1_I2CMasterDisableEcAm();

            /* Clean up the hardware before a transaction. */
            SCB_1_SetMasterInterruptMode    (SCB_1_NO_INTR_SOURCES);
            SCB_1_ClearMasterInterruptSource(SCB_1_INTR_MASTER_ALL);
            ...
            do
            {
                locStatus  = SCB_1_GetMasterInterruptSource() & SCB_1_INTR_MASTER_SEND_BYTE;
                locStatus |= SCB_1_GetSlaveInterruptSource() & SCB_1_SLAVE_INTR_I2C_ADDR;
                locStatus |= SCB_1_I2CMasterWaitOneUnit(&timeout);
            }
            while (0u == locStatus);

            /* Handle completion event and convert to status */
            errStatus = SCB_1_I2CMasterHandleStatus(locStatus);
        }
...
* \param bitRnW
* Direction of the following transfer. It is defined by read/write bit within
* address byte.See SCB_1_I2CMasterSendStart() function for constants.
*
* \param timeoutMs
* Defines in milliseconds the time that this function can block for.
* If that time expires, the function returns. If a zero is passed,
...
uint32 SCB_1_I2CMasterSendStop(uint32 timeoutMs)
{
    ...
    if(SCB_1_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = SCB_1_GetMasterInterruptSource() & SCB_1_INTR_MASTER_SEND_STOP;
            locStatus |= SCB_1_I2CMasterWaitOneUnit(&timeout);
        }
        while (0u == locStatus);
...
uint32 SCB_1_I2CMasterReadByte(uint32 ackNack, uint8 *rdByte, uint32 timeoutMs)
{
    ...
    if(SCB_1_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = SCB_1_GetMasterInterruptSource() & SCB_1_INTR_MASTER_RECEIVE_BYTE;
            rxNotEmpty = SCB_1_GetRxInterruptSource() & SCB_1_INTR_RX_NOT_EMPTY;
            locStatus |= SCB_1_I2CMasterWaitOneUnit(&timeout);
        }
        ...
        if (0u != rxNotEmpty)
        {
            *rdByte = (uint8)SCB_1_RX_FIFO_RD_REG;
            SCB_1_ClearRxInterruptSource(SCB_1_INTR_RX_ALL);
        }
        ...
        if (SCB_1_I2C_MSTR_NO_ERROR == errStatus)
        {
            if(SCB_1_I2C_ACK_DATA == ackNack)
            {
...
void SCB_1_I2CReStartGeneration(void)
{
    ...
    * In case of timeout the SCB_1_I2CMasterSendRestart handles this
    * case using own timeout and execute appropriate recovery.
    */

    const uint16 oneUnit = 1u;
    uint32 timeout = SCB_1_I2C_PHASE_GEN_TIMEOUT;

    /* Store direction of transfer */
    uint32 readDirection = SCB_1_I2C_STATUS_REG & SCB_1_I2C_STATUS_M_READ;

    /* Wait until slave releases SDA line: SCL tHIGH is complete */
    while ((SCB_1_WAIT_SDA_SET_HIGH) && (timeout > 0u))
    {
        CyDelayUs(oneUnit);
        --timeout;
    }
    ...
    {
        /* Refill timeout */
        timeout = SCB_1_I2C_PHASE_GEN_TIMEOUT;
        ...
        if (0u != readDirection)
        {
            ...
            SCB_1_SET_I2C_SCL_DR(SCB_1_I2C_SCL_LOW);
            CyDelayUs(SCB_1_I2C_TLOW_TIME);

            /* Drive tHIGH: SCL high phase */
            SCB_1_SET_I2C_SCL_DR(SCB_1_I2C_SCL_HIGH);
            CyDelayUs(SCB_1_I2C_THIGH_TIME);

            /* Wait until slave releases SDA line: SCL tHIGH is complete */
            while ((SCB_1_WAIT_SCL_SET_HIGH) && (timeout > 0u))
            {
                CyDelayUs(oneUnit);
                --timeout;
            }
...
/*******************************************************************************
* Function Name: SCB_1_I2CMasterHandleStatus
****************************************************************************//**
*
...
* SCB_1_I2C_MASTER_TIMEOUT and SCB_1_SLAVE_INTR_I2C_ADDR.
*
* \return
* Error status.
* See SCB_1_I2CMasterSendStart() function for constants.
...
static uint32 SCB_1_I2CMasterHandleStatus(uint32 status)
{
    ...
    if (0u != (status & SCB_1_I2C_MASTER_TIMEOUT))
    {
        retStatus  = SCB_1_I2C_MASTER_TIMEOUT;
        resetBlock = SCB_1_I2C_RESET_ERROR;
    }
    else if (0u != (SCB_1_SLAVE_INTR_I2C_ADDR & status))
    {
        /* Abort the master operation, the slave was addressed first */
        retStatus = SCB_1_I2C_MSTR_ERR_ABORT_START;

        SCB_1_I2C_MASTER_CMD_REG = 0u;

        SCB_1_state = SCB_1_I2C_FSM_IDLE;
    }
    else if (0u != (status & SCB_1_INTR_MASTER_I2C_ARB_LOST))
    {
        retStatus  = SCB_1_I2C_MSTR_ERR_ARB_LOST;

        if (SCB_1_CHECK_I2C_FSM_ADDR)
        {
            /* If slave is not enabled reset IP, otherwise give it a chance to match address */
            if (0u == (SCB_1_I2C_CTRL_REG & SCB_1_I2C_CTRL_SLAVE_MODE))
            {
                resetBlock = SCB_1_I2C_RESET_ERROR;
            }
    ...
    else if (0u != (status & SCB_1_INTR_MASTER_I2C_BUS_ERROR))
    {
        retStatus  = SCB_1_I2C_MSTR_ERR_BUS_ERR;
        resetBlock = SCB_1_I2C_RESET_ERROR;
    }
    else if (0u != (status & SCB_1_INTR_MASTER_I2C_NACK))
    {
        retStatus = SCB_1_I2C_MSTR_ERR_LB_NAK;
    }
    else /* Includes: INTR_MASTER_I2C_STOP and INTR_MASTER_I2C_ACK */
    {
        retStatus = SCB_1_I2C_MSTR_NO_ERROR;

        if (0u != (status & SCB_1_INTR_MASTER_I2C_STOP))
        {
        ...
        else
        {
            ...
            if (SCB_1_CHECK_I2C_FSM_ADDR)
            {
                ...
                SCB_1_state = (SCB_1_CHECK_I2C_FSM_RD) ?
                                          SCB_1_I2C_FSM_MSTR_RD_DATA : SCB_1_I2C_FSM_MSTR_WR_DATA;
            }
        }
    }

    if (SCB_1_I2C_RESET_ERROR == resetBlock)
    {
...
static uint32 SCB_1_I2CMasterWaitOneUnit(uint32 *timeout)
{
    ...
    if (*timeout > 0u)
    {
        ...
        if (0u == *timeout)
        {
            status = SCB_1_I2C_MASTER_TIMEOUT;
        }
    }
