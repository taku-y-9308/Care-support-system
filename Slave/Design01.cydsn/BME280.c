...
uint8_t BME280_readRegister(uint8_t offset)
{
	...
	switch (bme280.settings.commInterface) {
    	...
    	case BME280_I2C_MODE:
            ///I2C_Wakeup();
            SCB_1_I2CMasterClearStatus(); /* Clear any previous status */
            
            // Write to register
            SCB_1_I2CMasterWriteBuf(bme280.settings.I2CAddress, (uint8 *) &offset, 1, SCB_1_I2C_MODE_COMPLETE_XFER);
            for(iter = 0; iter < BME280_MAX_ITER; iter++)
            {
                if(0u != (SCB_1_I2CMasterStatus() & SCB_1_I2C_MSTAT_WR_CMPLT))
                {
            ...
            // Read Buffer        
            SCB_1_I2CMasterReadBuf(bme280.settings.I2CAddress, &result, numBytes, SCB_1_I2C_MODE_COMPLETE_XFER);
            
            for(iter = 0; iter < BME280_MAX_ITER; iter++){
                if(0u != (SCB_1_I2CMasterStatus() & SCB_1_I2C_MSTAT_RD_CMPLT))
                {
                    /* Transfer complete. Check Master status to make sure that transfer
                    completed without errors. */
                    break;
...
void BME280_writeRegister(uint8_t offset, uint8_t data)
{
	...
	switch (bme280.settings.commInterface)
	{
    	case BME280_I2C_MODE:
            ...
            // Write to register
            SCB_1_I2CMasterWriteBuf(bme280.settings.I2CAddress, (uint8 *) &packet, 2, SCB_1_I2C_MODE_COMPLETE_XFER);
            for(iter = 0; iter < BME280_MAX_ITER; iter++)
            {
                if(0u != (SCB_1_I2CMasterStatus() & SCB_1_I2C_MSTAT_WR_CMPLT))
                {
